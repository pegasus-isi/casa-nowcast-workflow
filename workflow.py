...
class CASAWorkflow(object):
    ...
    def generate_dax(self):
        ...
        for fn in self.forecast_fn:
            ...
            # extract time
            string_end = fn[-1].find(".")
            file_time = fn[-1][string_end - 12 : string_end]
            ...
            nowcast_split_job.add_args(fn[-1])
            nowcast_split_job.add_args(".")
            forecast_file = File(fn[-1])
            ...
            for x in range(31):
                ...
                pr_file = File(f"PredictedReflectivity_{x}min_{file_ymd}-{file_hms}.nc")
                pr_image = File(
                    f"PredictedReflectivity_{x}min_{file_ymd}-{file_hms}.png"
                )
                ...
                pr_image_job.add_outputs(
                    pr_image, stage_out=True, register_replica=False
                )
                pr_image_job.add_profiles(Namespace.PEGASUS, "label", f"label_{x}")
